...
use crate::rcore_fs::vfs::{FileSystem, FsError, INode, PollStatus, Metadata, FileType, FsInfo, Timespec};
use alloc::string::{String, ToString};
use core::any::Any;
use alloc::vec::Vec;
use spin::{RwLock, RwLockWriteGuard};
use crate::lkm::fs::FileSystemManager;
use core::cmp::min;
...
impl FileSystemType for RamFSBehav{
    fn mount(&self, source: &str, flags: u64, data: usize) -> Result<Arc<FileSystem>, FsError> {
        let root=Arc::new(RwLock::new(RamFSINode{
            this: None,
            parent: None,
        ...
        let mut root=downcast_inode(Arc::as_ref(&fs.root)).write();
...
fn downcast_inode<'a>(inode: &'a INode)->&'a RwLock<RamFSINode>{
    inode.as_any_ref().downcast_ref::<RwLock<RamFSINode>>().unwrap()
}
impl INode for RwLock<RamFSINode> {
    ...
    fn create(&self, name: &str, type_: FileType, mode: u32) -> Result<Arc<INode>, FsError> {
        ...
        if file.extra.type_ == FileType::Dir {
            ...
            let temp_file: Arc<INode> = Arc::new(RwLock::new(RamFSINode {
            ...
            let mut root = downcast_inode(Arc::as_ref(&temp_file)).write();
    ...
    fn link(&self, name: &str, other: &Arc<INode>) -> Result<(), FsError> {
        ...
        let mut locks = lockMultiple(&vec![self, other_lock]).into_iter();
    ...
    fn fs(&self) -> Arc<FileSystem> {
        Weak::upgrade(&self.read().fs).unwrap()
    }
