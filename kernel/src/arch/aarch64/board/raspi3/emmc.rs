...
use spin::Mutex;
use crate::lkm::cdev::{FileOperations, CharDev};
use crate::fs::{FileHandle, SeekFrom};
use crate::rcore_fs::vfs::{FsError, Metadata, PollStatus};
use alloc::string::String;
use crate::drivers::{Driver, DeviceType, BlockDriver};
use core::any::Any;
use crate::drivers::block::partition::PartitionBlockDriver;
use alloc::sync::Arc;
...
impl Driver for EMMCDriver{
    ...
    fn read_block(&self, block_id: usize, buf: &mut [u8]) -> bool {
        EMMC_CTL.lock().read(block_id as u32, 1, unsafe { slice::from_raw_parts_mut(buf.as_ptr() as *mut u32, 512 / 4) }).is_ok()
    }

    fn write_block(&self, block_id: usize, buf: &[u8]) -> bool {
        EMMC_CTL.lock().write(block_id as u32, 1, unsafe { slice::from_raw_parts_mut(buf.as_ptr() as *mut u32, 512 / 4) }).is_ok()
...
pub fn register_emmc(){
    if EMMC_CTL.lock().init() != 0 {
        panic!("emmc crashed");
    }
    let mut cdevman=crate::lkm::cdev::CDevManager::get().write();
    cdevman.registerDevice(65, CharDev{
        parent_module: None,
        file_op: Arc::new(PartitionBlockDriver::new(BlockDriver(Arc::new(EMMCDriver{}))))
    })
}